...
const benefits = [
	{
		...
		icon: <Target className="w-6 h-6 text-white sm:w-8 sm:h-8" />,
		bgColor: 'bg-blue-50',
		iconBg: 'bg-blue-500'
	},
	{
		...
		icon: <Zap className="w-6 h-6 text-white sm:w-8 sm:h-8" />,
		bgColor: 'bg-green-50',
		iconBg: 'bg-green-500'
	},
	{
		...
		icon: <Shield className="w-6 h-6 text-white sm:w-8 sm:h-8" />,
				...
				<div className="grid grid-cols-1 gap-8 mb-20 md:grid-cols-2 lg:grid-cols-4">
					{benefits.map((benefit, index) => (
						<motion.div
							...
							className={`group relative flex flex-col items-center text-center p-4 sm:p-6 rounded-2xl ${benefit.bgColor} border border-gray-200 hover:border-gray-300 shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer overflow-hidden`}
							whileHover={{
								y: -8,
								scale: 1.02,
						...
						>
							...
							{/* Icon */}
							<div className={`relative flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 rounded-2xl ${benefit.iconBg} mb-4 group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
								{benefit.icon}
							</div>

							{/* Content */}
							<h3 className="mb-3 text-lg font-bold text-gray-900 transition-colors duration-200 sm:text-xl group-hover:text-gray-800">
								{benefit.title}
							</h3>
