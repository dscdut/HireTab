...
import { Menu, X, Search, Briefcase, Headset, Contact, CreditCard } from 'lucide-react'
...
const Header = () => {
  ...
  const headerClasses = `fixed top-0 left-0 right-0 z-[100] transition-all duration-500 select-none ${isScrolled
    ? 'bg-white/90 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.06)] py-3 border-b border-gray-100'
    : 'bg-[#F5F5FF]/70 backdrop-blur-lg py-5 border-b border-[#3129B8]/10'
    }`
      ...
      <div className="container px-6 mx-auto lg:px-12">
        <div className="flex items-center justify-between">
          ...
          >
            <div className="relative">
              <img
                src={HireTabLogo}
                alt="HireTab"
                className={`transition-all duration-500 object-contain ${isScrolled ? 'w-12 h-12' : 'w-14 h-14'
                  } group-hover:scale-110`}
                draggable={false}
              />
              <div className="absolute inset-0 bg-[#3129B8] rounded-full blur-2xl opacity-0 group-hover:opacity-10 transition-opacity duration-500" />
            </div>
            <div className="flex flex-col">
              <span className={`font-black tracking-tight text-[#18191C] transition-all duration-500 ${isScrolled ? 'text-2xl' : 'text-3xl'
                }`}>
                Hiretab
              </span>
              <span className={`-mt-1 text-[11px] font-black uppercase tracking-widest text-[#3129B8] transition-all duration-500 ${isScrolled ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'
                }`}>
                Career Partner
              </span>
            </div>
          ...
          <nav className="hidden lg:flex items-center absolute left-1/2 -translate-x-1/2 space-x-1">
            {MENU_ITEMS.map((item, index) => (
              <Link
                key={index}
                to={item.href}
                className="relative px-6 py-2 text-[17px] font-bold text-[#515B6F] hover:text-[#3129B8] transition-colors duration-300 group"
              >
                ...
                <motion.div
                  className="absolute bottom-1 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-[#3129B8] rounded-full group-hover:w-1/2 transition-all duration-300"
                />
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button
              className="bg-[#3129B8] hover:bg-[#251E91] text-white px-8 py-2.5 text-[17px] font-bold rounded-xl transition-colors duration-300 active:scale-[0.98]"
            >
            ...
            >
              <div className="p-4 space-y-1">
                {MENU_ITEMS.map((item, index) => (
                  ...
                  >
                    <div className="w-10 h-10 bg-gray-50 rounded-lg flex items-center justify-center text-[#3129B8]">
                      <item.icon size={20} />
                    </div>
                ...
                <div className="pt-4 border-t border-gray-100 mt-2">
                  <Button
                    className="w-full bg-[#3129B8] py-6 text-base font-bold rounded-xl"
                  >
                    <Link to='/candidate/job'>Apply Now</Link>
                  </Button>
                </div>
